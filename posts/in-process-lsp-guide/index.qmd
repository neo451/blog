---
title: "A guide to building a in-process LSP in neovim"
author : "Zizhou Teng"
date: "2025-11-27"
categories: [neovim]
---
...
I have spent quite some time turning most parts of the functionality of [obsidian.nvim](https://github.com/obsidian-nvim/obsidian.nvim) into an in-process LSP, and many folks have asked me how it compares to an actually LSP process running outside of neovim, or just wanted to get a general idea of what it means, since there's not much good guides out there.

Below are my usual answers and I intend to use this post with an concrete example project to give you an idea of how it works and why I think it is my favorite "black magic" feature of neovim:

- You get neovim's default LSP keymaps for free:
    - you get meaningful keymaps that most users are already using, and by default are perfect keymaps to overload (expanded in [Definition and References](#definition-and-references))
- You let neovim to do the heavy lifting logic for you:
...
- You get to leverage the vast ecosystem of plugins that enhances the LSP experience:
...
- You get to "cheat" a bit than LSPs running as a system process:
    - they have to constantly keep negotiating with neovim about document's state, while you can just go `vim.api.nvim_buf_get_lines`

## Goal of this guide

I have been enjoying using [blink-cmp-words](https://github.com/archie-Judd/blink-cmp-words), that provides completion for words and synonyms when I write essays. But sometimes I get frustrated for not being able to hover over a word and get what I had just seen in the documentation window when doing completion. So building it into an LSP is the perfect solution.

However for the purpose of this guide, we will be ignoring all the details of actually querying the dictionary, and use various hacks and placeholders to get the idea across. I believe the eventual best solution would be using the same `WordNet` db approach that `blink-cmp-words` uses.
...
The most important API to know is [`:h vim.lsp.start`](https://neovim.io/doc/user/lsp.html#vim.lsp.start()), it takes `vim.lsp.ClientConfig` and the two fields that concerns us at the moment are:
...
First time seeing the signature name `vim.lsp.ClientConfig` for those not familiar with neovim LSP might be somewhat confusing: wasn't we writing a server here? The answer is neovim spawns one LSP client per running server. 
...
After spawning, the example above still will not work since our client and server have not negotiated a contract based on each other's capabilities, usual this is a two way process, but since we are not an editor-agnostic server, we don't really need to think about neovim's client capabilities. 
...
Let's try implement what sets me out to write this guide, the hover.
...
Isn't it amazing now you should be able to get hover windows on words?
...
My initial idea for a complete implementation will lookup the definitions and references, and then write a formatted page of markdown describing the meaning for each of the word in a tmp file, then return a list of `lsp.Location` pointing to them.


But since we did not proper implement `WordNet` queries, and the free dictionary API used above don't provide a mechanism to lookup synonyms, I came up with a rather fun and creative hack, that kind of bypasses neovim's handler pattern:
...
But I do think this is even better than what actually querying, at least to me browser would make more sense to display a definition of a word.

Some readers might ask if you did not use the `params` sent from client, and did not return anything to the client, instead you only did an arbitrary side effect that is completely unrelated to LSP, what difference does it have compared to just binding the function to the keys like this?
...
The following is basically an adaptation of [none-ls's spell source](https://github.com/nvimtools/none-ls.nvim/blob/1917c86818b5e058f53c2ea0ad38fc57534d62fc/lua/null-ls/builtins/completion/spell.lua). Funny story is I have seen more than one people posting on `r/neovim`, saying they get unwanted words in their completion list, because none-ls.nvim list this source as a default example, and many forgot they left it there.

The reason this source is not very useful is it uses vim's native `spellsuggest` function to complete the word. I actually happened to find this paragraph describing the difference between a spell file and a dictionary in help file today `:h spell-file-format`:
...
So as with all the cases above, we will need a capable local dictionary db or a paid dictionary API to really resolve this issue, but right now I think spell file approch is good enough for us, and I added fuzzy matching and filtering some unwanted cases to tidy up things a bit.
...
handlers[ms.textDocument_completion] = function(params, callback)
   ...
   local candidates = get_candidates(vim.fn.spellsuggest(word)) -- a real impelementation queries a dictionary here, also utilize the vim.fn.matchfuzzy
